...
int main() {
  ...
  //  PRINT(CAM_INTF_META_LENS_FOCUS_STATE,            pMetadata);
    ...
    /* Defining as int32_t so that this array is 4 bypMetadatae aligned */
    PRINT(CAM_INTF_META_PRIVATE_DATA,                pMetadata);

    /* Following are Params only and not metadata curpMetadataently */
    PRINT(CAM_INTF_PARM_HAL_VERSION,                 pMetadata);
    /* Shared between HALpMetadata and HAL3 */
